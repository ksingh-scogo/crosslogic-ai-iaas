...
function LaunchPage() {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-3xl font-bold text-gray-900">Launch GPU Instance</h1>
        <p className="text-gray-500 mt-1">Deploy an AI model on cloud GPUs</p>
      </div>
                ...
                <div className="grid gap-3 sm:grid-cols-2">
                  {models.slice(0, 6).map((model) => (
                    ...
                    >
                      ...
                      <p className="text-xs text-gray-500 mt-1">{model.family} • {model.size}</p>
              ...
              <div className="flex items-center justify-between rounded-lg border p-4">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-gray-500">Save 70-90% on costs</p>
                  </div>
                </div>
                <button
                  onClick={() => setConfig({ ...config, use_spot: !config.use_spot })}
                  className={`relative h-6 w-11 rounded-full transition-colors ${
                    config.use_spot ? 'bg-green-500' : 'bg-gray-300'
                  }`}
                >
                  <span
                    className={`absolute top-0.5 h-5 w-5 rounded-full bg-white shadow transition-transform ${
                      config.use_spot ? 'translate-x-5' : 'translate-x-0.5'
                    }`}
                  />
                </button>
              </div>
